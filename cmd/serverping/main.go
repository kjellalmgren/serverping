...
$ ./serverping -config ../../configs/default.json -http :8080 -log ../../logs/log.txt
*/
...
func main() {
	...
	config := serverping.NewConfig(jsonData)
	monitor := serverping.NewMonitor(config)
	go serverping.RunHttp(*address, monitor)
	monitor.Run()
}
